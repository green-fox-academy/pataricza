import static org.junit.Assert.*;

import org.junit.Test;

import java.util.Arrays;

public class ExtensionTest {

  Extension extension = new Extension();

  @Test
  public void testAdd_2and3is5() {
    assertEquals(5, extension.add(2, 3));
  }

  @Test
  public void testAdd_1and4is5() {
    assertEquals(5, extension.add(1, 4));
  }

  @Test
  public void testAdd_2and6is8() {
    assertEquals(8, extension.add(2,6));
  }

  @Test
  public void testMaxOfThree_first() {
    assertEquals(5, extension.maxOfThree(5, 4, 3));
  }

  @Test
  public void testMaxOfThree_third() {
    assertEquals(5, extension.maxOfThree(3, 4, 5));
  }

  @Test
  public void testMaxOfThree_second() {
    assertEquals(5, extension.maxOfThree(3, 5, 4));
  }

  @Test
  public void testMedian_four() {
    assertEquals(6, extension.median(Arrays.asList(7,5,7,6)), 0.00001);
  }

  @Test
  public void testMedian_many() {
    assertEquals(5, extension.median(Arrays.asList(1,2,3,4,5,7,13,6,8)), 0.00001);
  }

  @Test
  public void testMedian_double() {
    assertEquals(2.5, extension.median(Arrays.asList(1,2,3,4)), 0.00001);
  }

  @Test
  public void testIsVowel_a() {
    assertTrue(extension.isVowel('a'));
  }

  @Test
  public void testIsVowel_u() {
    assertTrue(extension.isVowel('u'));
  }

  @Test
  public void testTranslate_bemutatkozik() {
    assertEquals("bevemuvutavatkovozivik", extension.translate("bemutatkozik"));
  }

  @Test
  public void testTranslate_lagopus() {
    assertEquals("lavagovopuvus", extension.translate("lagopus"));
  }

  @Test
  public void testTranslate_teve() {
    assertEquals("teveveve", extension.translate("teve"));
  }

  @Test
  public void testTranslate_tea() {
    assertEquals("teveava", extension.translate("tea"));
  }
}